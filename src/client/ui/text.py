...
class Text:
    ...
    def format(self):
        ...
        if self.text.startswith(TextFormatting.JUMBLE):
            self.instance.new_task("jumble-txt-text", self.jumble_task)
        elif self.text.startswith(TextFormatting.UNDERLINE):
            pass
        elif self.text.startswith(TextFormatting.ITALIC):
            self.set_text(self.text.split(TextFormatting.ITALIC)[1])
            self.node.setSlant(0.3)
        elif self.text.startswith(TextFormatting.SHAKE):
            self.set_text(self.text.split(TextFormatting.SHAKE)[1])
            self.instance.new_task("shake-text", self.shake_task)
        elif self.text.startswith(TextFormatting.BOLD):
            self.set_text(
                f"\1black\1{self.get_text().split(TextFormatting.BOLD)[1]}\2\2")
        elif self.text.startswith(TextFormatting.WAVE):
            self.set_text(self.text.split(TextFormatting.WAVE)[1])
            self.instance.new_task('wave-text', self.wave_task)
        elif self.text.startswith(TextFormatting.RGB):
            self.setText(self.text.split(TextFormatting.RGB)[1])
            self.instance.new_task('rgb_text_task', self.rgb_task)
    ...
    def rgb_task(self, task):
        ...
        """
        # nifty comprehension.
        color_rgb_list = [math.sin((self.instance.get_time_elapsed(
        ) * x) - self.instance.get_dt()) for x in range(3)]
    ...
    def getPos(self, attr: str = None):
        ...
        if attr:
            if attr.lower() not in ('x', 'y', 'z'):
                raise ValueError(
                    "Expected x, y or z for attribute get, got %s".format(attr))
    ...
    def tween_to(self, pos: Vector3):
        self.nodePath.loop(pos.to_lvec3f())
    ...
    def set_pos(self, position=(0, 0, 0), virtual: bool = False):
        """
    ...
    def setPos(self, *args): return self.set_pos(*args)

    def set_scale(self, scale: float = 0.01):
        """
    ...
    def setScale(self, *args): return self.set_scale(*args)
    ...
    def setColor(self, *args): return self.set_color(*args)

    def set_text(self, text: str = "Hello, World!"):
        """
        Set the `text` in this Text object to something.
        """
        if self.nodePath.is_empty():
            return
        # This text label is hidden; ignore this request.
        if self.text != self.node.getText():
            return
    ...
    def setText(self, *args): return self.set_text(*args)
