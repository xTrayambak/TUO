...
class IOFile:
    ...
    def serialized_write(self, data: object, stype: SerializationType = SerializationType.COMPRESSED_BYTES):
        """
        Serialize the data and then write it to this file.

        [required]

        `data` -- the data to write.
        """
        if self.get_mode() != 'wb': self.change_mode('wb')
        ser = Serializer()
        self.write(ser.serialize(data, stype))


    def serialized_read(self, stype: SerializationType = SerializationType.COMPRESSED_BYTES) -> object:
        """
        Read data from this file, and turn it into an object/dict via the built-in serializer.
        """
        if self.get_mode() != 'rb': self.change_mode('rb')
        ser = Serializer()
        return ser.deserialize(self.read(), stype)
