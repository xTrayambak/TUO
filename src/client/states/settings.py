...
def settings_page(instance, previous_state: int = 1):
    """
    The settings page. This is an actual atrocity. I refuse to touch this ever again. WHY DID I LOVE CAMELCASE SO MUCH? HAS I EVER?
    """
    instance.clear()

    basicFont = instance.font_loader.load("gentium_basic")
    ...
    def ao_toggle_func():
        settings['video']['ao'] = not settings['video']['ao']
        dump_setting(settings)

        ao_toggle.set_text(f'Ambient Occlusion [{BOOL_TO_STR[get_setting("video", "ao")]}]')

        # TODO: This ends up reloading each and every visual effect, converts data to JSON and dumps it, causing quite the lag. Maybe fix it?
        # Maybe? Just maaaaaaybe! :P
    ...
    ao_toggle = Button(
        instance, f'Ambient Occlusion [{BOOL_TO_STR[get_setting("video", "ao")]}]',
        pos = (-0.2, 0, 0.1), scale = 0.5, parent = videoFrame, command = ao_toggle_func
    )
